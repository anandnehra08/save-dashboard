...
def render_students_module():
    ...
    tab1, tab2, tab3 = st.tabs(["📝 New Admission", "📋 Live Student Directory", "🔍 Search & Manage"])
    ...
    with tab3:
        ...
        search_sr = st.number_input("Enter SR No to Search", min_value=1, step=1)
        if st.button("🔍 Search Student"):
            if supabase:
                ...
                if res.data:
                    student = res.data[0]
                    st.json(student)
                    if st.button(f"🗑️ Delete Record SR: {search_sr}"):
